	...
	for (std::vector<BattleUnit*>::iterator j = _save->getUnits()->begin(); j != _save->getUnits()->end(); ++j)
	{
		...
		if ((*j)->getStatus() != STATUS_DEAD)
		{
			if ((*j)->getHealth() <= 0)
			{
				...
				{
					int modifier = _save->getUnitMoraleModifier(victim); // Return always 100 anyway since the victim is civilian.
					int loserModCivil = _save->getUnitMoraleModifier(victim); // Return always 100 anyway since the victim is civilian.
					int loserModXCOM = _save->getUnitMoraleModifier(victim); // Returns x-com high ranked morale resist bonus, 125 MAX.				
					...
					for (std::vector<BattleUnit*>::iterator i = _save->getUnits()->begin(); i != _save->getUnits()->end(); ++i)
					{
						if (!(*i)->isOut() && (*i)->getArmor()->getSize() == 1)
						{
							...
							//--- end of Extended Civilian
							// the winning squad all get a morale increase
							else if ((*i)->getOriginalFaction() == FACTION_HOSTILE) // Killed civilian increases morale of aliens.
							{
								(*i)->moraleChange(10 * winnerMod / 100);
...
bool BattlescapeGame::handlePanickingPlayer()
{
	for (std::vector<BattleUnit*>::iterator j = _save->getUnits()->begin(); j != _save->getUnits()->end(); ++j)
	{
		...
		//--- end of Extend civilian behaviour
	}
	return true;
}
